      ...
      <InnerLayout>
        ...
        >
          <View className="flex-row items-center bg-white">
            <Image source={{ uri: profile?.profileImageUrl }} className="w-[54] h-[54] rounded-[12px] mr-3" />
        ...
        <View className="mt-4 bg-white">
          {settingsItems.map((item, index) => (
            <React.Fragment key={item.key}>
              <SettingItem label={item.label} onPress={() => navigation.navigate(item.key)} />
              {index < settingsItems.length - 1 && <View className="h-[1px] bg-gray-100 mx-4" />}
            </React.Fragment>
          ))}
        </View>
      </InnerLayout>
